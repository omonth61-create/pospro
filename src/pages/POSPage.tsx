...
const POSPage: React.FC = () => {
  ...
  const handleFinishCheckout = async (customerName: string, paidAmount: number = 0) => {
    ...
    try {
      ...
      const trxId = await indexdbTransaksi.create(getTotal(), items);

      // Jika ada customer dan pembayaran kurang dari total, catat piutang
      const total = getTotal();
      const paid = Number(paidAmount) || 0;
      if (customerName && paid < total) {
        try {
          const now = Date.now();
          await indexdbDebt.save({
            ...
            amount: total,
            paidAmount: paid,
            description: `Penjualan ${trxId}`,
            dueDate: now + 7 * 24 * 60 * 60 * 1000, // default jatuh tempo 7 hari
            status: paid >= total ? 'paid' : paid > 0 ? 'partial' : 'unpaid',
            created_at: now,
            updated_at: now,
          });
        } catch (e) {
          console.error('Gagal menyimpan piutang:', e);
        }
      }
    } catch (error) {
      console.error(error);
            ...
            >
              <div className="p-8 text-center">
                ...
                {isAdminUser ? (
                  ...
                  <p className="text-[10px] font-bold text-red-500 uppercase tracking-widest leading-normal">Kasir pegawai tidak diizinkan menambah barang baru. Hubungi administrator toko.</p>
                )}
              </div>
              <div className="p-8 bg-slate-50/50 border-t border-slate-50 flex gap-4">
                {!isAdminUser ? (
                  <button 
                    ...
                    className="w-full py-5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-3xl font-black text-sm uppercase tracking-widest transition-all"
                  >
                    OK, COBA LAGI
                  </button>
                ) : (
