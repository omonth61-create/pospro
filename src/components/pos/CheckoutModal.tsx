...
interface Props {
  onClose: () => void;
  onConfirm: (customerName: string, paidAmount: number) => void;
}
            ...
            <div className="space-y-8">
              <div className="flex gap-2">
                <button type="button" onClick={() => setPaymentMethod('cash')}
                  className={cn(
                    "flex-1 py-3 rounded-2xl font-black text-sm",
                    paymentMethod === 'cash' ? 'bg-white text-slate-700 border-2 border-slate-100' : 'bg-slate-50 text-slate-400'
                  )}
                >Tunai</button>
                <button type="button" onClick={() => setPaymentMethod('credit')}
                  className={cn(
                    "flex-1 py-3 rounded-2xl font-black text-sm",
                    paymentMethod === 'credit' ? 'bg-white text-slate-700 border-2 border-slate-100' : 'bg-slate-50 text-slate-400'
                  )}
                >Bayar Nanti</button>
              </div>
               <div className={cn(
        ...
        <div className="p-8 bg-slate-50/50 border-t border-slate-50 gap-4 flex flex-col">
          ...
          {step === 'payment' && (
            <div className="flex gap-4">
               ...
               <button 
                 disabled={paymentMethod === 'cash' ? cash < total : false}
